...
const waitForConfirmations = async (connection, signature) => {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const createTransaction = async (fromWallet, toAddress, data) => {
    ...
    const transaction = new Transaction().add(
      SystemProgram.transfer({
        ...
        lamports: 0.0000001 * 1e9, // Example amount in lamports (1 SOL = 1e9 lamports)
      }),
      instruction
    );
  ...
  const handleDeployClick = async (data, dataToSave) => {    
    ...
    try {
      const transaction = await createTransaction(fromWallet, publicKey.toString(), data);
      ...
      const confirmedTransaction = await waitForConfirmations(connection, signature);
